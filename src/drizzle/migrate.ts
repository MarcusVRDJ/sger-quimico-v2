...
async function ensureChecklistTemplateSchema(db: ReturnType<typeof drizzle>) {
  await db.execute(sql`
    DO $$ BEGIN
      CREATE TYPE "public"."status_revisao_checklist" AS ENUM('RASCUNHO', 'PENDENTE_APROVACAO', 'APROVADO', 'REJEITADO', 'ARQUIVADO');
    EXCEPTION
  ...
  await db.execute(sql`
    DO $$ BEGIN
      CREATE TYPE "public"."tipo_checklist" AS ENUM('RECEBIMENTO', 'EXPEDICAO');
    EXCEPTION
      WHEN duplicate_object THEN null;
    END $$;
  `);
  ...
  await db.execute(sql`
    CREATE UNIQUE INDEX IF NOT EXISTS "checklist_template_revisoes_template_versao_unique"
    ON "checklist_template_revisoes" USING btree ("template_id", "versao");
  `);
}
...
async function run() {
  ...
  try {
    ...
    await ensureChecklistTemplateSchema(db);
